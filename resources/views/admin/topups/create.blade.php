    ...
    <div class="py-12 bg-zinc-50 dark:bg-zinc-950 min-h-screen">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-8">
                ...
                <h1 class="text-3xl font-black text-gray-950 dark:text-white tracking-tight">Create Credit Bundle</h1>
                <p class="text-gray-500 dark:text-gray-400 text-sm mt-1">Offer one-time token purchases to your users.</p>
            </div>

            <form action="{{ route('admin.topup-packages.store') }}" method="POST" class="space-y-6">
                @csrf

                <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-[32px] p-10 shadow-sm space-y-8">
                    <div class="space-y-2">
                        <label class="block text-[11px] font-black text-gray-400 uppercase tracking-widest">Bundle Label (e.g. Starter Pack)</label>
                        <input type="text" name="name" required class="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-2xl px-5 py-4 text-gray-950 dark:text-white font-bold focus:border-indigo-500 focus:ring-0">
                    </div>
                    ...
                    <label class="flex items-center gap-4 p-5 bg-zinc-50 dark:bg-zinc-950 rounded-2xl border border-dashed border-zinc-200 dark:border-zinc-800 cursor-pointer hover:border-indigo-500/50 transition-all">
                        <input type="checkbox" name="is_active" value="1" checked class="w-6 h-6 rounded-lg border-zinc-300 dark:border-zinc-800 text-indigo-600 focus:ring-indigo-500 bg-transparent">
                        <div>
                            <p class="text-sm font-black text-gray-950 dark:text-white uppercase tracking-tight leading-none">Visible to Users</p>
                            <p class="text-[10px] text-gray-400 mt-1 font-bold">Uncheck to hide this bundle from the marketplace without deleting it.</p>
                        </div>
                    </label>
                </div>
