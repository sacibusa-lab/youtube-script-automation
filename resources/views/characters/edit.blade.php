<x-app-layout>
    <div class="max-w-2xl mx-auto space-y-8">
        ...
        <div>
            ...
            <h1 class="text-[34px] font-black text-gray-900 dark:text-white leading-none">Edit Character</h1>
            <p class="text-gray-500 dark:text-gray-400 mt-2">Update {{ $character->name }}'s visual DNA.</p>
        </div>
        ...
        <form action="{{ route('characters.update', $character) }}" method="POST" class="space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-[28px] border border-gray-100 dark:border-gray-700 p-8 space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $character->name) }}" required maxlength="100"
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" required maxlength="1000"
                        class="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-600 rounded-xl px-4 py-3 text-gray-900 dark:text-white font-medium focus:ring-2 focus:ring-teal-500 focus:border-transparent transition resize-none">{{ old('description', $character->description) }}</textarea>
                    @error('description') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">Associated Niche</label>
                    <input type="text" name="niche" value="{{ old('niche', $character->niche) }}"
                        class="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-600 rounded-xl px-4 py-3 text-gray-900 dark:text-white font-medium focus:ring-2 focus:ring-teal-500 focus:border-transparent transition">
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-[28px] border border-gray-100 dark:border-gray-700 p-8 space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    @foreach([
                        ['key' => 'age', 'label' => 'Age / Age Range'],
                        ['key' => 'ethnicity', 'label' => 'Ethnicity / Skin Tone'],
                        ['key' => 'hair', 'label' => 'Hair'],
                        ['key' => 'eyes', 'label' => 'Eyes'],
                        ['key' => 'build', 'label' => 'Body Build'],
                        ['key' => 'style', 'label' => 'Clothing Style'],
                    ] as $field)
                    <div>
                        <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">{{ $field['label'] }}</label>
                        <input type="text" name="visual_traits[{{ $field['key'] }}]"
                            value="{{ old('visual_traits.'.$field['key'], $character->visual_traits[$field['key']] ?? '') }}"
                ...
                <div>
                    ...
                    <input type="text" name="visual_traits[signature_detail]"
                        value="{{ old('visual_traits.signature_detail', $character->visual_traits['signature_detail'] ?? '') }}"
                        class="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-600 rounded-xl px-4 py-3 text-gray-900 dark:text-white font-medium text-sm focus:ring-2 focus:ring-teal-500 focus:border-transparent transition">
                </div>
            </div>
            ...
            <div class="flex items-center gap-4">
                <button type="submit" class="flex-1 bg-teal-600 hover:bg-teal-700 text-white px-8 py-4 rounded-2xl font-black text-[15px] uppercase tracking-widest transition-all hover:-translate-y-0.5">
                    Save Changes
                </button>
                <a href="{{ route('characters.index') }}" class="px-6 py-4 rounded-2xl text-gray-500 hover:text-gray-700 font-bold transition">
                    Cancel
                </a>
            </div>
        </form>
